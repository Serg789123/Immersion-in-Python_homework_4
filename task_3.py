...
RICHNESS_TAX = decimal.Decimal(10) / decimal.Decimal(10000) # налог
...
def richnes():
    ...
    print(f'Удержан налог на богатство {RICHNESS_TAX}% в размере {percent} у.е.\n'
          f'Итого на карте омталось {account} у.е.')
def deposit_withdraw():
    global account
    global count
    ammount = 1
    while ammount % 50 != 0:
        ammount = int(input(f'Введите сумму кратную {MUTIPLICIRY}: '))
    if command == CMD_DEPOSIT:
        ...
        if account >= ammount + withdraw_tax:
            ...
            print(f'Снятие скарты {ammount} у.е.\nКомиссия за снятие {withdraw_tax}\n'
